...
class Jaccard():
    ...
    def exact_match(self,khspbunmber,boat_number,customer_number):   # 输入：客户商品编号、船舶编号、客户账号
        a = Preprocess()
        ...
        if (len(yuce_number_only) == 1):
            ...
            for l in index:
                if (boat_number == boat_number_match[l]):  # 根据船舶编号匹配
            ...
            if (flag01 == 0):  # 根据客户账号去匹配
                for l in index:
                    if (customer_number == customer_number_match[l]):
                        yuce_number_all.append(wuliaonumber[l])
                        m = 1
                        break
...
a = Jaccard()
print(a.exact_match('510787','SN027562','C000100008'))
